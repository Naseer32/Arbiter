...
""".strip()
    ...
    def _maybe_resolve_parent(self, job: Job) -> None:
        """
        If this job is a milestone child and just resolved, check whether
        every sibling milestone under the same parent has also resolved.
        If so, roll the parent job's status up to "resolved" too -- purely
        a status rollup for an at-a-glance view. The parent itself never
        holds or pays out funds; each milestone is an independent job with
        its own escrow, settled through the exact same functions as any
        other job.
        """
        ...
        for other in self.jobs:
            if other.parent_job_id == job.parent_job_id and other.status != "resolved":
                return
    ...
    # ---------- Requester: post a milestone job (added path, existing ----------
    # ---------- create_job() above is untouched and still works alone) --------

    @gl.public.write.payable
    def create_milestone_job(self, worker: str, specs: list[str], amounts: list[u256]) -> u256:
        """
        Creates a parent "container" job plus one independent child job per
        milestone. Each child is a completely normal job -- submit_work,
        approve, dispute, appeal, finalize, recover_unavailable_job, and
        abandon_job all operate on it exactly as they would on any
        single-payout job created via create_job(), since none of those
        functions were changed. The parent's status ("milestones_open") is
        distinct from every status those functions check for, so they
        naturally refuse to act on the parent job without needing any new
        guard logic.
        """
        total = gl.message.value

        if len(specs) < 2:
            raise gl.vm.UserError(
                "milestone jobs need at least 2 milestones -- use create_job for a single-payout job"
            )
        ...
        for a in amounts:
            if a == u256(0):
                raise gl.vm.UserError("each milestone amount must be > 0")
            sum_amounts += int(a)

        if u256(sum_amounts) != total:
            raise gl.vm.UserError(
                f"sum of milestone amounts ({sum_amounts}) must equal escrowed value ({int(total)})"
            )
        ...
        now = datetime.datetime.now()
        requester = gl.message.sender_address
        worker_addr = Address(worker)
        summary_spec = " | ".join(f"Milestone {i + 1}: {s}" for i, s in enumerate(specs))
    ...
    def get_milestones(self, parent_job_id: u256) -> list[u256]:
        """Returns the 1-based job ids of every milestone child under the
        given parent job id, in creation order."""
        result = []
        for idx, j in enumerate(self.jobs):
            if j.parent_job_id == parent_job_id:
                result.append(u256(idx + 1))
        return result
